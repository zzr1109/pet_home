    ...
    <div style="display: flex;margin: 10px 0">
      <div style="flex: 3;margin-right: 10px">
        <el-carousel trigger="click" height="350px">
          <el-carousel-item v-for="item in carouselData" :key="item.id">
            <el-image style="width: 100%;height: 100%" :src="baseURL() + item.imageUrlList" fit="cover"></el-image>
          </el-carousel-item>
        </el-carousel>
      </div>
      <div style="flex: 2;display: flex;flex-direction: column;background-color: #fff">
        <p style="display: flex;padding: 5px">
          ...
          <span style="align-self: center;color: var(--accent-200); margin-left: 10px;font-size: 18px;font-weight: bold">推荐宠物</span>
        </p>
        <div class="hiddenScrollbar" style="height: 300px;overflow-y: scroll">
          ...
          >
            <div style="display: flex;align-items: center">
              <el-image :src="baseURL() + item.menuImages[0]" style="width: 60px;height: 60px;"></el-image>
              <div style="display: flex;flex-direction: column;height: 60px;margin-left: 5px">
                                <span style="font-size: 14px;font-weight: bold">{{item.petName}}({{ item.petAge }})</span>
        ...
        <div style="margin-bottom: 5px">
          <div style="display: flex;justify-content: space-between">
            <p style="display: flex;padding: 5px;padding-left: 0">
              <img src="@/assets/images/推荐宠物.png" style="height: 30px;width: 30px"/>
              <span style="align-self: center;color: var(--accent-200); margin-left: 10px;font-size: 18px;font-weight: bold">宠物合集</span>
            </p>
            <el-radio-group @change="getPetList" v-model="pageRequest.q.petCategoryId">
              <el-radio-button label="全" value="null"></el-radio-button>
              <el-radio-button v-for="item in petCategoryList" :key="item.id" :label="item.category" :value="item.id"></el-radio-button>
            </el-radio-group>
          </div>
        </div>
